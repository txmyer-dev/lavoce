...
class CaptureExtractRequest(BaseModel):
    """Request to run the intelligence-extraction pipeline on a capture.

    ``model_size`` selects the LLM; defaults server-side to the remote
    FreeLLMAPI backend. ``dispatch_todoist`` opts into live task creation —
    off by default, since creating tasks is an outward-facing write.
    """

    model_size: Optional[str] = Field(
        None, pattern="^(0\\.6B|1\\.7B|4B|freellmapi-remote)$"
    )
    dispatch_todoist: bool = False


class TodoistDispatchResult(BaseModel):
    """Outcome of the Todoist sink for one extraction."""
...
class CaptureExtractResponse(BaseModel):
    """Structured intelligence extracted from a capture, plus where it landed."""
    ...
    llm_model: str
    note_path: str
    todoist: TodoistDispatchResult


class CaptureSettingsResponse(BaseModel):
    """Server-persisted defaults for the capture / refine flow."""

    # This server is CPU-only and offloads both STT and LLM work to remote
    # providers, so no on-device weights are downloaded. STT defaults to
    # Microsoft Speech (Azure fast transcription) and refinement to the remote
    # FreeLLMAPI backend. Both are gated on configuration, not a local download.
    stt_model: str = Field(default="microsoft-stt", pattern="^(base|small|medium|large|turbo|microsoft-stt)$")
    language: str = Field(default="auto")
    auto_refine: bool = True
    llm_model: str = Field(default="freellmapi-remote", pattern="^(0\\.6B|1\\.7B|4B|freellmapi-remote)$")
    smart_cleanup: bool = True
    self_correction: bool = True
    preserve_technical: bool = True
...
class CaptureSettingsUpdate(BaseModel):
    ...
    stt_model: Optional[str] = Field(default=None, pattern="^(base|small|medium|large|turbo|microsoft-stt)$")
    language: Optional[str] = None
    auto_refine: Optional[bool] = None
    llm_model: Optional[str] = Field(default=None, pattern="^(0\\.6B|1\\.7B|4B|freellmapi-remote)$")
