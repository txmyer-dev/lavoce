  ...
  async setupWindowCloseHandler(): Promise<void> {
    try {
      ...
      await listen<null>('window-close-requested', async () => {
        ...
        const keepRunning = state.keepServerRunningOnClose;
        const runInBackground = state.runInBackground;

        if (runInBackground) {
          console.log('[lifecycle] window-close-requested: runInBackground=true, hiding window');
